...
while True:
    ...
    for j in range(len(bpoints)):
            ...
            for k in range(1, len(bpoints[j])):
                ...
                check_movement(x=bpoints[j][k][0], ox=bpoints[j][k - 1][0], sensativity=100)
                cv2.line(frame, bpoints[j][k - 1], bpoints[j][k], colors[0], 2)
                cv2.line(paintWindow, bpoints[j][k - 1], bpoints[j][k], colors[0], 2)
